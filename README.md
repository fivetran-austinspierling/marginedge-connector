...
Also confirmed in live data: some MarginEdge IDs are human-readable codes rather than opaque tokens (e.g. a `vendorItemCode` like `"...CHOCOLATE_CUPCAKE_W/_CARAMEL_BC..."`) and can contain characters - `/` in this case - that are reserved in a URL path. Every ID this connector splices into a request URL (`order_id`, `ccpid`, `vendor_id`, `vendor_item_code`, `countsheet_id`, `inventory_id`, `section_id`) is now run through `_url_path_segment()` (percent-encoding via `urllib.parse.quote`) before being interpolated, so a `/` or other reserved character in an ID can't turn into extra, unintended path segments - which previously caused a 403 from MarginEdge's gateway on a route it couldn't match, rather than a clear error about the ID itself.
