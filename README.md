...
Confirmed in live client data: some nested list items have a null (or blank-string) natural ID field even though MarginEdge's documented examples always show one populated (e.g. a product's category allocation can include an "unallocated" remainder with no `categoryId`). Since the destination rejects a null primary key outright, every field sourced from a list item's own ID that feeds a primary key (`vendor_id`, `company_concept_product_id`, `countsheet_id`, `inventory_id`, `order_id`, plus every nested-array PK: `product_categories`, `profit_and_loss_report_categories`, `sales_report_categories`, `order_attachments`, `vendor_item_packaging`, `vendor_items`, `vendor_items_by_product`, `countsheet_sections`, `inventory_sections`, `inventory_section_items`) runs through `_is_blank()`/`_fallback_id()`, which substitutes a stable `__unassigned_<index>` value (based on position in the source array) whenever the real ID is missing, so the row is still captured instead of crashing the sync.

### Special characters in path-parameter IDs

Also confirmed in live data: some MarginEdge IDs are human-readable codes rather than opaque tokens (e.g. a `vendorItemCode` like `"...CHOCOLATE_CUPCAKE_W/_CARAMEL_BC..."`) and can contain characters - `/` in this case - that are reserved in a URL path. Every ID this connector splices into a request URL (`order_id`, `ccpid`, `vendor_id`, `vendor_item_code`, `countsheet_id`, `inventory_id`, `section_id`) is run through `_url_path_segment()` (percent-encoding via `urllib.parse.quote`) before being interpolated, so a `/` or other reserved character can't silently turn into extra, unintended path segments.

For `vendor_item_code` specifically, percent-encoding alone isn't enough: MarginEdge's API gateway rejects an encoded slash (`%2F`) in this path segment with a 400 regardless (a well-known AWS API Gateway limitation - it decodes the path before route-matching, so `%2F` still looks like an extra segment to it). There is no way to reach the packaging endpoint for a vendor item code containing `/`, so `_sync_vendor_item_packaging()` detects this upfront and skips that one item's packaging lookup with a warning, rather than letting one uncooperative code abort the whole sync. If the same 400-on-encoded-slash pattern turns up on another path ID (`order_id`, `ccpid`, etc.), the same skip-with-warning approach should be applied there too.
